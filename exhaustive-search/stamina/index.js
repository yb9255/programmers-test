function solution(k, dungeons) {
  ...
  const getMaxCount = (stamina, count) => {
    maxCount = Math.max(maxCount, count);

    for (let i = 0; i < dungeons.length; i++) {
      if (visited[i]) continue;

      const [needed, consumed] = dungeons[i];
      if (stamina < needed) continue;
      if (stamina < consumed) continue;

      visited[i] = true;
      getMaxCount(stamina - consumed, count + 1);
      visited[i] = false;
    }
  };
